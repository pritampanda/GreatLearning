...
select cus_gender as Gender,count(cus_name) as Count from customer as c
inner join `order` as o on c.cus_id = o.cus_id 
...
in (select p.supp_id from product_details as p group by p.supp_id having count(p.supp_id)>1) 
...
select product.pro_id, product.pro_name from product,`order` ,product_details
